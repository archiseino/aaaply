  ...
  return (
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4'>
      ...
      >
          ...
          {activeTab === 'api' ? (
            <div className='flex flex-col gap-6'>
              <div className='flex flex-col gap-2'>
                ...
                <p className='text-xs' style={{ color: 'var(--text-muted)' }}>
                  Tempel key Anda di sini jika ingin menggunakan kuota pribadi.
                </p>
              </div>
              ...
              <div className='flex flex-col gap-3'>
                <label
                  ...
                  style={{ color: 'var(--text-secondary)' }}
                >
                  Integrasi Email Draf (Zero Friction)
                </label>
                <label className='flex items-center gap-3 cursor-pointer'>
                  <input
                    type='checkbox'
                    checked={gmailApiEnabled}
                    onChange={(e) => setGmailApiEnabled(e.target.checked)}
                  ...
                  >
                    Aktifkan Integrasi Draf Gmail API
                  </span>
                </label>
                <label className='flex items-center gap-3 cursor-pointer'>
                  <input
                    type='checkbox'
                    checked={outlookApiEnabled}
                    onChange={(e) => setOutlookApiEnabled(e.target.checked)}
                    className='w-4 h-4 rounded'
                  />
                  <span
                    className='text-sm'
                    style={{ color: 'var(--text-primary)' }}
                  >
                    Aktifkan Integrasi Draf Outlook API
                  </span>
                </label>
                <p
              ...
              <div className='flex flex-col gap-2'>
                ...
                <input
                  ...
                  className='w-full rounded-lg px-4 py-2.5 font-mono text-sm focus:outline-none focus:ring-1 transition-all'
                  style={inputStyle}
                  placeholder='1a2B3cD4eF5g6H7iJ8kL9mN'
                />
                <p className='text-xs' style={{ color: 'var(--text-muted)' }}>
                  Buat Google Sheet, share dengan service account, lalu tempel
                  ID Sheet di sini. Gunakan tombol Sync di halaman Tracker.
                </p>
                <p className='text-xs' style={{ color: 'var(--status-blue)' }}>
                  <a
                    href='https://docs.google.com/spreadsheets/d/1F49h57ESanoiLs2wUapjVRKGv1FkTAhOm85GLabKoRQ/'
                    target='_blank'
              ...
              <div className='flex flex-col gap-2'>
                ...
                />
                <p className='text-xs' style={{ color: 'var(--text-muted)' }}>
                  Sel pertama tempat data dimulai (contoh: B7 jika header di
                  baris 5, data mulai baris 7 kolom B).
                </p>
              </div>
            </div>
          ) : (
            <div className='flex flex-col gap-4'>
              ...
              <div className='flex flex-col gap-2 mt-2'>
                ...
                {cvHistory.length === 0 ? (
                  <div
                    className='text-center p-4 border border-dashed rounded-lg text-sm'
                    style={{
                      borderColor: 'var(--border)',
                      color: 'var(--text-muted)',
                    }}
        ...
        <div
          className='p-5 flex justify-end shrink-0'
          style={{
